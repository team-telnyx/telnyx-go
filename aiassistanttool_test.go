...
func TestAIAssistantToolAdd(t *testing.T) {
	...
	_, err := client.AI.Assistants.Tools.Add(
		context.TODO(),
		"tool_id",
		telnyx.AIAssistantToolAddParams{
...
func TestAIAssistantToolRemove(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
...
func TestAIAssistantToolTestWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
