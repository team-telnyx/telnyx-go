...
func TestTexmlAccountGetRecordingsJsonWithOptionalParams(t *testing.T) {
	...
	_, err := client.Texml.Accounts.GetRecordingsJson(
		...
		telnyx.TexmlAccountGetRecordingsJsonParams{
			DateCreated: telnyx.Time(time.Now()),
			Page:        telnyx.Int(0),
...
func TestTexmlAccountGetTranscriptionsJsonWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
