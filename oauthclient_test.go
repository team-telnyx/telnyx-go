...
func TestOAuthClientNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.OAuthClients.New(context.TODO(), telnyx.OAuthClientNewParams{
...
func TestOAuthClientGet(t *testing.T) {
	...
	_, err := client.OAuthClients.Get(context.TODO(), "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
	if err != nil {
		var apierr *telnyx.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
...
func TestOAuthClientListWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.OAuthClients.List(context.TODO(), telnyx.OAuthClientListParams{
		FilterAllowedGrantTypesContains: telnyx.OAuthClientListParamsFilterAllowedGrantTypesContainsClientCredentials,
		FilterClientID:                  telnyx.String("filter[client_id]"),
		FilterClientType:                telnyx.OAuthClientListParamsFilterClientTypeConfidential,
		FilterName:                      telnyx.String("filter[name]"),
		FilterNameContains:              telnyx.String("filter[name][contains]"),
		FilterVerified:                  telnyx.Bool(true),
