...
func TestQueueNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.Queues.New(context.TODO(), telnyx.QueueNewParams{
		QueueName: "tier_1_support",
		MaxSize:   telnyx.Int(100),
	})
	if err != nil {
...
func TestQueueGet(t *testing.T) {
	...
	_, err := client.Queues.Get(context.TODO(), "queue_name")
	if err != nil {
		var apierr *telnyx.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
...
func TestQueueListWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Queues.List(context.TODO(), telnyx.QueueListParams{
