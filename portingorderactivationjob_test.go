...
func TestPortingOrderActivationJobGet(t *testing.T) {
...
func TestPortingOrderActivationJobUpdateWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.PortingOrders.ActivationJobs.Update(
		...
		telnyx.PortingOrderActivationJobUpdateParams{
			ID:         "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
			ActivateAt: telnyx.Time(time.Now()),
		},
	)
	if err != nil {
		var apierr *telnyx.Error
...
func TestPortingOrderActivationJobListWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
