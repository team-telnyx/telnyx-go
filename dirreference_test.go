...
func TestDirReferenceSubmitWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Dir.References.Submit(
		...
		telnyx.DirReferenceSubmitParams{
			BusinessReferences: []telnyx.DirReferenceSubmitParamsBusinessReference{{
			...
			FinancialReference: telnyx.DirReferenceSubmitParamsFinancialReference{
				Email:                    "dana.reyes@example.com",
				FullName:                 "Dana Reyes",
				PhoneE164:                "+14155550123",
				Timezone:                 "America/New_York",
