...
func TestAdvancedOrderUpdateWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.AdvancedOrders.Update(
		context.TODO(),
		"182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
		telnyx.AdvancedOrderUpdateParams{
			AreaCode:           telnyx.String("xxx"),
			Comments:           telnyx.String("comments"),
			CountryCode:        telnyx.String("xx"),
			CustomerReference:  telnyx.String("customer_reference"),
			Features:           []string{"sms"},
			PhoneNumberType:    telnyx.AdvancedOrderUpdateParamsPhoneNumberTypeLocal,
			Quantity:           telnyx.Int(1),
			RequirementGroupID: telnyx.String("3fa85f64-5717-4562-b3fc-2c963f66afa6"),
		},
	)
