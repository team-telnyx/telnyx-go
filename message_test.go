...
func TestMessageWhatsappWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Messages.Whatsapp(context.TODO(), telnyx.MessageWhatsappParams{
		From: "+13125551234",
		To:   "+13125551234",
		...
		Type:               telnyx.MessageWhatsappParamsTypeWhatsapp,
		WebhookURL:         telnyx.String("webhook_url"),
	})
	if err != nil {
		var apierr *telnyx.Error
