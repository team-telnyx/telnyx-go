...
func TestEmailMessageListWithOptionalParams(t *testing.T) {
	...
	_, err := client.EmailMessages.List(context.TODO(), telnyx.EmailMessageListParams{
		FilterMetadata: telnyx.String("filter[metadata]"),
		FilterTags:     telnyx.String("filter[tags]"),
...
func TestEmailMessageUpdateSchedule(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.EmailMessages.UpdateSchedule(
		context.TODO(),
		"182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
