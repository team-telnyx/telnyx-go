/*
Telnyx API

SIP trunking, SMS, MMS, Call Control and Telephony Data Services.

API version: 2.0.0
Contact: support@telnyx.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package telnyx

import (
	"encoding/json"
)

// checks if the TexmlCreateSiprecSessionResponseBody type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &TexmlCreateSiprecSessionResponseBody{}

// TexmlCreateSiprecSessionResponseBody struct for TexmlCreateSiprecSessionResponseBody
type TexmlCreateSiprecSessionResponseBody struct {
	// The id of the account the resource belongs to.
	AccountSid *string `json:"account_sid,omitempty"`
	// The id of the call the resource belongs to.
	CallSid *string `json:"call_sid,omitempty"`
	// The SID of the siprec session.
	Sid *string `json:"sid,omitempty"`
	// The date and time the siprec session was created.
	DateCreated *string `json:"date_created,omitempty"`
	// The date and time the siprec session was last updated.
	DateUpdated *string `json:"date_updated,omitempty"`
	// The date and time the siprec session was started.
	StartTime *string `json:"start_time,omitempty"`
	// The status of the siprec session.
	Status *string `json:"status,omitempty"`
	// The track used for the siprec session.
	Track *string `json:"track,omitempty"`
	// The URI of the siprec session.
	Uri *string `json:"uri,omitempty"`
	// The error code of the siprec session.
	ErrorCode *string `json:"error_code,omitempty"`
}

// NewTexmlCreateSiprecSessionResponseBody instantiates a new TexmlCreateSiprecSessionResponseBody object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewTexmlCreateSiprecSessionResponseBody() *TexmlCreateSiprecSessionResponseBody {
	this := TexmlCreateSiprecSessionResponseBody{}
	return &this
}

// NewTexmlCreateSiprecSessionResponseBodyWithDefaults instantiates a new TexmlCreateSiprecSessionResponseBody object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewTexmlCreateSiprecSessionResponseBodyWithDefaults() *TexmlCreateSiprecSessionResponseBody {
	this := TexmlCreateSiprecSessionResponseBody{}
	return &this
}

// GetAccountSid returns the AccountSid field value if set, zero value otherwise.
func (o *TexmlCreateSiprecSessionResponseBody) GetAccountSid() string {
	if o == nil || IsNil(o.AccountSid) {
		var ret string
		return ret
	}
	return *o.AccountSid
}

// GetAccountSidOk returns a tuple with the AccountSid field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TexmlCreateSiprecSessionResponseBody) GetAccountSidOk() (*string, bool) {
	if o == nil || IsNil(o.AccountSid) {
		return nil, false
	}
	return o.AccountSid, true
}

// HasAccountSid returns a boolean if a field has been set.
func (o *TexmlCreateSiprecSessionResponseBody) HasAccountSid() bool {
	if o != nil && !IsNil(o.AccountSid) {
		return true
	}

	return false
}

// SetAccountSid gets a reference to the given string and assigns it to the AccountSid field.
func (o *TexmlCreateSiprecSessionResponseBody) SetAccountSid(v string) {
	o.AccountSid = &v
}

// GetCallSid returns the CallSid field value if set, zero value otherwise.
func (o *TexmlCreateSiprecSessionResponseBody) GetCallSid() string {
	if o == nil || IsNil(o.CallSid) {
		var ret string
		return ret
	}
	return *o.CallSid
}

// GetCallSidOk returns a tuple with the CallSid field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TexmlCreateSiprecSessionResponseBody) GetCallSidOk() (*string, bool) {
	if o == nil || IsNil(o.CallSid) {
		return nil, false
	}
	return o.CallSid, true
}

// HasCallSid returns a boolean if a field has been set.
func (o *TexmlCreateSiprecSessionResponseBody) HasCallSid() bool {
	if o != nil && !IsNil(o.CallSid) {
		return true
	}

	return false
}

// SetCallSid gets a reference to the given string and assigns it to the CallSid field.
func (o *TexmlCreateSiprecSessionResponseBody) SetCallSid(v string) {
	o.CallSid = &v
}

// GetSid returns the Sid field value if set, zero value otherwise.
func (o *TexmlCreateSiprecSessionResponseBody) GetSid() string {
	if o == nil || IsNil(o.Sid) {
		var ret string
		return ret
	}
	return *o.Sid
}

// GetSidOk returns a tuple with the Sid field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TexmlCreateSiprecSessionResponseBody) GetSidOk() (*string, bool) {
	if o == nil || IsNil(o.Sid) {
		return nil, false
	}
	return o.Sid, true
}

// HasSid returns a boolean if a field has been set.
func (o *TexmlCreateSiprecSessionResponseBody) HasSid() bool {
	if o != nil && !IsNil(o.Sid) {
		return true
	}

	return false
}

// SetSid gets a reference to the given string and assigns it to the Sid field.
func (o *TexmlCreateSiprecSessionResponseBody) SetSid(v string) {
	o.Sid = &v
}

// GetDateCreated returns the DateCreated field value if set, zero value otherwise.
func (o *TexmlCreateSiprecSessionResponseBody) GetDateCreated() string {
	if o == nil || IsNil(o.DateCreated) {
		var ret string
		return ret
	}
	return *o.DateCreated
}

// GetDateCreatedOk returns a tuple with the DateCreated field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TexmlCreateSiprecSessionResponseBody) GetDateCreatedOk() (*string, bool) {
	if o == nil || IsNil(o.DateCreated) {
		return nil, false
	}
	return o.DateCreated, true
}

// HasDateCreated returns a boolean if a field has been set.
func (o *TexmlCreateSiprecSessionResponseBody) HasDateCreated() bool {
	if o != nil && !IsNil(o.DateCreated) {
		return true
	}

	return false
}

// SetDateCreated gets a reference to the given string and assigns it to the DateCreated field.
func (o *TexmlCreateSiprecSessionResponseBody) SetDateCreated(v string) {
	o.DateCreated = &v
}

// GetDateUpdated returns the DateUpdated field value if set, zero value otherwise.
func (o *TexmlCreateSiprecSessionResponseBody) GetDateUpdated() string {
	if o == nil || IsNil(o.DateUpdated) {
		var ret string
		return ret
	}
	return *o.DateUpdated
}

// GetDateUpdatedOk returns a tuple with the DateUpdated field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TexmlCreateSiprecSessionResponseBody) GetDateUpdatedOk() (*string, bool) {
	if o == nil || IsNil(o.DateUpdated) {
		return nil, false
	}
	return o.DateUpdated, true
}

// HasDateUpdated returns a boolean if a field has been set.
func (o *TexmlCreateSiprecSessionResponseBody) HasDateUpdated() bool {
	if o != nil && !IsNil(o.DateUpdated) {
		return true
	}

	return false
}

// SetDateUpdated gets a reference to the given string and assigns it to the DateUpdated field.
func (o *TexmlCreateSiprecSessionResponseBody) SetDateUpdated(v string) {
	o.DateUpdated = &v
}

// GetStartTime returns the StartTime field value if set, zero value otherwise.
func (o *TexmlCreateSiprecSessionResponseBody) GetStartTime() string {
	if o == nil || IsNil(o.StartTime) {
		var ret string
		return ret
	}
	return *o.StartTime
}

// GetStartTimeOk returns a tuple with the StartTime field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TexmlCreateSiprecSessionResponseBody) GetStartTimeOk() (*string, bool) {
	if o == nil || IsNil(o.StartTime) {
		return nil, false
	}
	return o.StartTime, true
}

// HasStartTime returns a boolean if a field has been set.
func (o *TexmlCreateSiprecSessionResponseBody) HasStartTime() bool {
	if o != nil && !IsNil(o.StartTime) {
		return true
	}

	return false
}

// SetStartTime gets a reference to the given string and assigns it to the StartTime field.
func (o *TexmlCreateSiprecSessionResponseBody) SetStartTime(v string) {
	o.StartTime = &v
}

// GetStatus returns the Status field value if set, zero value otherwise.
func (o *TexmlCreateSiprecSessionResponseBody) GetStatus() string {
	if o == nil || IsNil(o.Status) {
		var ret string
		return ret
	}
	return *o.Status
}

// GetStatusOk returns a tuple with the Status field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TexmlCreateSiprecSessionResponseBody) GetStatusOk() (*string, bool) {
	if o == nil || IsNil(o.Status) {
		return nil, false
	}
	return o.Status, true
}

// HasStatus returns a boolean if a field has been set.
func (o *TexmlCreateSiprecSessionResponseBody) HasStatus() bool {
	if o != nil && !IsNil(o.Status) {
		return true
	}

	return false
}

// SetStatus gets a reference to the given string and assigns it to the Status field.
func (o *TexmlCreateSiprecSessionResponseBody) SetStatus(v string) {
	o.Status = &v
}

// GetTrack returns the Track field value if set, zero value otherwise.
func (o *TexmlCreateSiprecSessionResponseBody) GetTrack() string {
	if o == nil || IsNil(o.Track) {
		var ret string
		return ret
	}
	return *o.Track
}

// GetTrackOk returns a tuple with the Track field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TexmlCreateSiprecSessionResponseBody) GetTrackOk() (*string, bool) {
	if o == nil || IsNil(o.Track) {
		return nil, false
	}
	return o.Track, true
}

// HasTrack returns a boolean if a field has been set.
func (o *TexmlCreateSiprecSessionResponseBody) HasTrack() bool {
	if o != nil && !IsNil(o.Track) {
		return true
	}

	return false
}

// SetTrack gets a reference to the given string and assigns it to the Track field.
func (o *TexmlCreateSiprecSessionResponseBody) SetTrack(v string) {
	o.Track = &v
}

// GetUri returns the Uri field value if set, zero value otherwise.
func (o *TexmlCreateSiprecSessionResponseBody) GetUri() string {
	if o == nil || IsNil(o.Uri) {
		var ret string
		return ret
	}
	return *o.Uri
}

// GetUriOk returns a tuple with the Uri field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TexmlCreateSiprecSessionResponseBody) GetUriOk() (*string, bool) {
	if o == nil || IsNil(o.Uri) {
		return nil, false
	}
	return o.Uri, true
}

// HasUri returns a boolean if a field has been set.
func (o *TexmlCreateSiprecSessionResponseBody) HasUri() bool {
	if o != nil && !IsNil(o.Uri) {
		return true
	}

	return false
}

// SetUri gets a reference to the given string and assigns it to the Uri field.
func (o *TexmlCreateSiprecSessionResponseBody) SetUri(v string) {
	o.Uri = &v
}

// GetErrorCode returns the ErrorCode field value if set, zero value otherwise.
func (o *TexmlCreateSiprecSessionResponseBody) GetErrorCode() string {
	if o == nil || IsNil(o.ErrorCode) {
		var ret string
		return ret
	}
	return *o.ErrorCode
}

// GetErrorCodeOk returns a tuple with the ErrorCode field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TexmlCreateSiprecSessionResponseBody) GetErrorCodeOk() (*string, bool) {
	if o == nil || IsNil(o.ErrorCode) {
		return nil, false
	}
	return o.ErrorCode, true
}

// HasErrorCode returns a boolean if a field has been set.
func (o *TexmlCreateSiprecSessionResponseBody) HasErrorCode() bool {
	if o != nil && !IsNil(o.ErrorCode) {
		return true
	}

	return false
}

// SetErrorCode gets a reference to the given string and assigns it to the ErrorCode field.
func (o *TexmlCreateSiprecSessionResponseBody) SetErrorCode(v string) {
	o.ErrorCode = &v
}

func (o TexmlCreateSiprecSessionResponseBody) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o TexmlCreateSiprecSessionResponseBody) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.AccountSid) {
		toSerialize["account_sid"] = o.AccountSid
	}
	if !IsNil(o.CallSid) {
		toSerialize["call_sid"] = o.CallSid
	}
	if !IsNil(o.Sid) {
		toSerialize["sid"] = o.Sid
	}
	if !IsNil(o.DateCreated) {
		toSerialize["date_created"] = o.DateCreated
	}
	if !IsNil(o.DateUpdated) {
		toSerialize["date_updated"] = o.DateUpdated
	}
	if !IsNil(o.StartTime) {
		toSerialize["start_time"] = o.StartTime
	}
	if !IsNil(o.Status) {
		toSerialize["status"] = o.Status
	}
	if !IsNil(o.Track) {
		toSerialize["track"] = o.Track
	}
	if !IsNil(o.Uri) {
		toSerialize["uri"] = o.Uri
	}
	if !IsNil(o.ErrorCode) {
		toSerialize["error_code"] = o.ErrorCode
	}
	return toSerialize, nil
}

type NullableTexmlCreateSiprecSessionResponseBody struct {
	value *TexmlCreateSiprecSessionResponseBody
	isSet bool
}

func (v NullableTexmlCreateSiprecSessionResponseBody) Get() *TexmlCreateSiprecSessionResponseBody {
	return v.value
}

func (v *NullableTexmlCreateSiprecSessionResponseBody) Set(val *TexmlCreateSiprecSessionResponseBody) {
	v.value = val
	v.isSet = true
}

func (v NullableTexmlCreateSiprecSessionResponseBody) IsSet() bool {
	return v.isSet
}

func (v *NullableTexmlCreateSiprecSessionResponseBody) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableTexmlCreateSiprecSessionResponseBody(val *TexmlCreateSiprecSessionResponseBody) *NullableTexmlCreateSiprecSessionResponseBody {
	return &NullableTexmlCreateSiprecSessionResponseBody{value: val, isSet: true}
}

func (v NullableTexmlCreateSiprecSessionResponseBody) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableTexmlCreateSiprecSessionResponseBody) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


