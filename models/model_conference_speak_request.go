/*
Telnyx API

SIP trunking, SMS, MMS, Call Control and Telephony Data Services.

API version: 2.0.0
Contact: support@telnyx.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package telnyx

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the ConferenceSpeakRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ConferenceSpeakRequest{}

// ConferenceSpeakRequest struct for ConferenceSpeakRequest
type ConferenceSpeakRequest struct {
	// Call Control IDs of participants who will hear the spoken text. When empty all participants will hear the spoken text.
	CallControlIds []string `json:"call_control_ids,omitempty"`
	// The text or SSML to be converted into speech. There is a 3,000 character limit.
	Payload string `json:"payload"`
	// The type of the provided payload. The payload can either be plain text, or Speech Synthesis Markup Language (SSML).
	PayloadType *string `json:"payload_type,omitempty"`
	// Specifies the voice used in speech synthesis.  - Define voices using the format `<Provider>.<Model>.<VoiceId>`. Specifying only the provider will give default values for voice_id and model_id.   **Supported Providers:** - **AWS:** Use `AWS.Polly.<VoiceId>` (e.g., `AWS.Polly.Joanna`). For neural voices, which provide more realistic, human-like speech, append `-Neural` to the `VoiceId` (e.g., `AWS.Polly.Joanna-Neural`). For long-form engine, append `-LongForm` to the `VoiceId` (e.g., `AWS.Polly.Danielle-LongForm`). For generative engine, the latest provided by AWS Polly, append `-Generative` to the `VoiceId` (e.g., `AWS.Polly.Danielle-Generative`). Check the [available voices](https://docs.aws.amazon.com/polly/latest/dg/available-voices.html) for compatibility. - **Azure:** Use `Azure.<VoiceId>. (e.g. Azure.en-CA-ClaraNeural, Azure.en-CA-LiamNeural, Azure.en-US-BrianMultilingualNeural, Azure.en-US-Ava:DragonHDLatestNeural. For a complete list of voices, go to [Azure Voice Gallery](https://speech.microsoft.com/portal/voicegallery).) - **ElevenLabs:** Use `ElevenLabs.<ModelId>.<VoiceId>` (e.g., `ElevenLabs.eleven_multilingual_v2.21m00Tcm4TlvDq8ikWAM`). The `ModelId` part is optional. To use ElevenLabs, you must provide your ElevenLabs API key as an integration identifier secret in `\"voice_settings\": {\"api_key_ref\": \"<secret_identifier>\"}`. See [integration secrets documentation](https://developers.telnyx.com/api/secrets-manager/integration-secrets/create-integration-secret) for details. Check [available voices](https://elevenlabs.io/docs/api-reference/get-voices).  - **Telnyx:** Use `Telnyx.<model_id>.<voice_id>`  For service_level basic, you may define the gender of the speaker (male or female).
	Voice string `json:"voice"`
	VoiceSettings *ElevenLabsVoiceSettings `json:"voice_settings,omitempty"`
	// The language you want spoken. This parameter is ignored when a `Polly.*` voice is specified.
	Language *string `json:"language,omitempty"`
	// Use this field to avoid execution of duplicate commands. Telnyx will ignore subsequent commands with the same `command_id` as one that has already been executed.
	CommandId *string `json:"command_id,omitempty"`
}

type _ConferenceSpeakRequest ConferenceSpeakRequest

// NewConferenceSpeakRequest instantiates a new ConferenceSpeakRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewConferenceSpeakRequest(payload string, voice string) *ConferenceSpeakRequest {
	this := ConferenceSpeakRequest{}
	this.Payload = payload
	var payloadType string = "text"
	this.PayloadType = &payloadType
	this.Voice = voice
	return &this
}

// NewConferenceSpeakRequestWithDefaults instantiates a new ConferenceSpeakRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewConferenceSpeakRequestWithDefaults() *ConferenceSpeakRequest {
	this := ConferenceSpeakRequest{}
	var payloadType string = "text"
	this.PayloadType = &payloadType
	return &this
}

// GetCallControlIds returns the CallControlIds field value if set, zero value otherwise.
func (o *ConferenceSpeakRequest) GetCallControlIds() []string {
	if o == nil || IsNil(o.CallControlIds) {
		var ret []string
		return ret
	}
	return o.CallControlIds
}

// GetCallControlIdsOk returns a tuple with the CallControlIds field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConferenceSpeakRequest) GetCallControlIdsOk() ([]string, bool) {
	if o == nil || IsNil(o.CallControlIds) {
		return nil, false
	}
	return o.CallControlIds, true
}

// HasCallControlIds returns a boolean if a field has been set.
func (o *ConferenceSpeakRequest) HasCallControlIds() bool {
	if o != nil && !IsNil(o.CallControlIds) {
		return true
	}

	return false
}

// SetCallControlIds gets a reference to the given []string and assigns it to the CallControlIds field.
func (o *ConferenceSpeakRequest) SetCallControlIds(v []string) {
	o.CallControlIds = v
}

// GetPayload returns the Payload field value
func (o *ConferenceSpeakRequest) GetPayload() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Payload
}

// GetPayloadOk returns a tuple with the Payload field value
// and a boolean to check if the value has been set.
func (o *ConferenceSpeakRequest) GetPayloadOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Payload, true
}

// SetPayload sets field value
func (o *ConferenceSpeakRequest) SetPayload(v string) {
	o.Payload = v
}

// GetPayloadType returns the PayloadType field value if set, zero value otherwise.
func (o *ConferenceSpeakRequest) GetPayloadType() string {
	if o == nil || IsNil(o.PayloadType) {
		var ret string
		return ret
	}
	return *o.PayloadType
}

// GetPayloadTypeOk returns a tuple with the PayloadType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConferenceSpeakRequest) GetPayloadTypeOk() (*string, bool) {
	if o == nil || IsNil(o.PayloadType) {
		return nil, false
	}
	return o.PayloadType, true
}

// HasPayloadType returns a boolean if a field has been set.
func (o *ConferenceSpeakRequest) HasPayloadType() bool {
	if o != nil && !IsNil(o.PayloadType) {
		return true
	}

	return false
}

// SetPayloadType gets a reference to the given string and assigns it to the PayloadType field.
func (o *ConferenceSpeakRequest) SetPayloadType(v string) {
	o.PayloadType = &v
}

// GetVoice returns the Voice field value
func (o *ConferenceSpeakRequest) GetVoice() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Voice
}

// GetVoiceOk returns a tuple with the Voice field value
// and a boolean to check if the value has been set.
func (o *ConferenceSpeakRequest) GetVoiceOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Voice, true
}

// SetVoice sets field value
func (o *ConferenceSpeakRequest) SetVoice(v string) {
	o.Voice = v
}

// GetVoiceSettings returns the VoiceSettings field value if set, zero value otherwise.
func (o *ConferenceSpeakRequest) GetVoiceSettings() ElevenLabsVoiceSettings {
	if o == nil || IsNil(o.VoiceSettings) {
		var ret ElevenLabsVoiceSettings
		return ret
	}
	return *o.VoiceSettings
}

// GetVoiceSettingsOk returns a tuple with the VoiceSettings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConferenceSpeakRequest) GetVoiceSettingsOk() (*ElevenLabsVoiceSettings, bool) {
	if o == nil || IsNil(o.VoiceSettings) {
		return nil, false
	}
	return o.VoiceSettings, true
}

// HasVoiceSettings returns a boolean if a field has been set.
func (o *ConferenceSpeakRequest) HasVoiceSettings() bool {
	if o != nil && !IsNil(o.VoiceSettings) {
		return true
	}

	return false
}

// SetVoiceSettings gets a reference to the given ElevenLabsVoiceSettings and assigns it to the VoiceSettings field.
func (o *ConferenceSpeakRequest) SetVoiceSettings(v ElevenLabsVoiceSettings) {
	o.VoiceSettings = &v
}

// GetLanguage returns the Language field value if set, zero value otherwise.
func (o *ConferenceSpeakRequest) GetLanguage() string {
	if o == nil || IsNil(o.Language) {
		var ret string
		return ret
	}
	return *o.Language
}

// GetLanguageOk returns a tuple with the Language field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConferenceSpeakRequest) GetLanguageOk() (*string, bool) {
	if o == nil || IsNil(o.Language) {
		return nil, false
	}
	return o.Language, true
}

// HasLanguage returns a boolean if a field has been set.
func (o *ConferenceSpeakRequest) HasLanguage() bool {
	if o != nil && !IsNil(o.Language) {
		return true
	}

	return false
}

// SetLanguage gets a reference to the given string and assigns it to the Language field.
func (o *ConferenceSpeakRequest) SetLanguage(v string) {
	o.Language = &v
}

// GetCommandId returns the CommandId field value if set, zero value otherwise.
func (o *ConferenceSpeakRequest) GetCommandId() string {
	if o == nil || IsNil(o.CommandId) {
		var ret string
		return ret
	}
	return *o.CommandId
}

// GetCommandIdOk returns a tuple with the CommandId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConferenceSpeakRequest) GetCommandIdOk() (*string, bool) {
	if o == nil || IsNil(o.CommandId) {
		return nil, false
	}
	return o.CommandId, true
}

// HasCommandId returns a boolean if a field has been set.
func (o *ConferenceSpeakRequest) HasCommandId() bool {
	if o != nil && !IsNil(o.CommandId) {
		return true
	}

	return false
}

// SetCommandId gets a reference to the given string and assigns it to the CommandId field.
func (o *ConferenceSpeakRequest) SetCommandId(v string) {
	o.CommandId = &v
}

func (o ConferenceSpeakRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ConferenceSpeakRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.CallControlIds) {
		toSerialize["call_control_ids"] = o.CallControlIds
	}
	toSerialize["payload"] = o.Payload
	if !IsNil(o.PayloadType) {
		toSerialize["payload_type"] = o.PayloadType
	}
	toSerialize["voice"] = o.Voice
	if !IsNil(o.VoiceSettings) {
		toSerialize["voice_settings"] = o.VoiceSettings
	}
	if !IsNil(o.Language) {
		toSerialize["language"] = o.Language
	}
	if !IsNil(o.CommandId) {
		toSerialize["command_id"] = o.CommandId
	}
	return toSerialize, nil
}

func (o *ConferenceSpeakRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"payload",
		"voice",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varConferenceSpeakRequest := _ConferenceSpeakRequest{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varConferenceSpeakRequest)

	if err != nil {
		return err
	}

	*o = ConferenceSpeakRequest(varConferenceSpeakRequest)

	return err
}

type NullableConferenceSpeakRequest struct {
	value *ConferenceSpeakRequest
	isSet bool
}

func (v NullableConferenceSpeakRequest) Get() *ConferenceSpeakRequest {
	return v.value
}

func (v *NullableConferenceSpeakRequest) Set(val *ConferenceSpeakRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableConferenceSpeakRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableConferenceSpeakRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableConferenceSpeakRequest(val *ConferenceSpeakRequest) *NullableConferenceSpeakRequest {
	return &NullableConferenceSpeakRequest{value: val, isSet: true}
}

func (v NullableConferenceSpeakRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableConferenceSpeakRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


