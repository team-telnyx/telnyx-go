/*
Telnyx API

SIP trunking, SMS, MMS, Call Control and Telephony Data Services.

API version: 2.0.0
Contact: support@telnyx.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package telnyx

import (
	"encoding/json"
)

// checks if the ElevenLabsVoiceSettings type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ElevenLabsVoiceSettings{}

// ElevenLabsVoiceSettings struct for ElevenLabsVoiceSettings
type ElevenLabsVoiceSettings struct {
	// The `identifier` for an integration secret [/v2/integration_secrets](https://developers.telnyx.com/api/secrets-manager/integration-secrets/create-integration-secret) that refers to your ElevenLabs API key. Warning: Free plans are unlikely to work with this integration.
	ApiKeyRef *string `json:"api_key_ref,omitempty"`
}

// NewElevenLabsVoiceSettings instantiates a new ElevenLabsVoiceSettings object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewElevenLabsVoiceSettings() *ElevenLabsVoiceSettings {
	this := ElevenLabsVoiceSettings{}
	return &this
}

// NewElevenLabsVoiceSettingsWithDefaults instantiates a new ElevenLabsVoiceSettings object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewElevenLabsVoiceSettingsWithDefaults() *ElevenLabsVoiceSettings {
	this := ElevenLabsVoiceSettings{}
	return &this
}

// GetApiKeyRef returns the ApiKeyRef field value if set, zero value otherwise.
func (o *ElevenLabsVoiceSettings) GetApiKeyRef() string {
	if o == nil || IsNil(o.ApiKeyRef) {
		var ret string
		return ret
	}
	return *o.ApiKeyRef
}

// GetApiKeyRefOk returns a tuple with the ApiKeyRef field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ElevenLabsVoiceSettings) GetApiKeyRefOk() (*string, bool) {
	if o == nil || IsNil(o.ApiKeyRef) {
		return nil, false
	}
	return o.ApiKeyRef, true
}

// HasApiKeyRef returns a boolean if a field has been set.
func (o *ElevenLabsVoiceSettings) HasApiKeyRef() bool {
	if o != nil && !IsNil(o.ApiKeyRef) {
		return true
	}

	return false
}

// SetApiKeyRef gets a reference to the given string and assigns it to the ApiKeyRef field.
func (o *ElevenLabsVoiceSettings) SetApiKeyRef(v string) {
	o.ApiKeyRef = &v
}

func (o ElevenLabsVoiceSettings) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ElevenLabsVoiceSettings) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.ApiKeyRef) {
		toSerialize["api_key_ref"] = o.ApiKeyRef
	}
	return toSerialize, nil
}

type NullableElevenLabsVoiceSettings struct {
	value *ElevenLabsVoiceSettings
	isSet bool
}

func (v NullableElevenLabsVoiceSettings) Get() *ElevenLabsVoiceSettings {
	return v.value
}

func (v *NullableElevenLabsVoiceSettings) Set(val *ElevenLabsVoiceSettings) {
	v.value = val
	v.isSet = true
}

func (v NullableElevenLabsVoiceSettings) IsSet() bool {
	return v.isSet
}

func (v *NullableElevenLabsVoiceSettings) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableElevenLabsVoiceSettings(val *ElevenLabsVoiceSettings) *NullableElevenLabsVoiceSettings {
	return &NullableElevenLabsVoiceSettings{value: val, isSet: true}
}

func (v NullableElevenLabsVoiceSettings) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableElevenLabsVoiceSettings) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


