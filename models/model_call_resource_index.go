/*
Telnyx API

SIP trunking, SMS, MMS, Call Control and Telephony Data Services.

API version: 2.0.0
Contact: support@telnyx.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package telnyx

import (
	"encoding/json"
)

// checks if the CallResourceIndex type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CallResourceIndex{}

// CallResourceIndex struct for CallResourceIndex
type CallResourceIndex struct {
	Calls []CallResource `json:"calls,omitempty"`
	// The number of the last element on the page, zero-indexed.
	End *int32 `json:"end,omitempty"`
	// /v2/texml/Accounts/61bf923e-5e4d-4595-a110-56190ea18a1b/Calls.json?Page=0&PageSize=1
	FirstPageUri *string `json:"first_page_uri,omitempty"`
	// /v2/texml/Accounts/61bf923e-5e4d-4595-a110-56190ea18a1b/Calls.json?Page=1&PageSize=1&PageToken=MTY4AjgyNDkwNzIxMQ
	NextPageUri *string `json:"next_page_uri,omitempty"`
	// Current page number, zero-indexed.
	Page *int32 `json:"page,omitempty"`
	// The number of items on the page
	PageSize *int32 `json:"page_size,omitempty"`
	// The number of the first element on the page, zero-indexed.
	Start *int32 `json:"start,omitempty"`
	// The URI of the current page.
	Uri *string `json:"uri,omitempty"`
}

// NewCallResourceIndex instantiates a new CallResourceIndex object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCallResourceIndex() *CallResourceIndex {
	this := CallResourceIndex{}
	return &this
}

// NewCallResourceIndexWithDefaults instantiates a new CallResourceIndex object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCallResourceIndexWithDefaults() *CallResourceIndex {
	this := CallResourceIndex{}
	return &this
}

// GetCalls returns the Calls field value if set, zero value otherwise.
func (o *CallResourceIndex) GetCalls() []CallResource {
	if o == nil || IsNil(o.Calls) {
		var ret []CallResource
		return ret
	}
	return o.Calls
}

// GetCallsOk returns a tuple with the Calls field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CallResourceIndex) GetCallsOk() ([]CallResource, bool) {
	if o == nil || IsNil(o.Calls) {
		return nil, false
	}
	return o.Calls, true
}

// HasCalls returns a boolean if a field has been set.
func (o *CallResourceIndex) HasCalls() bool {
	if o != nil && !IsNil(o.Calls) {
		return true
	}

	return false
}

// SetCalls gets a reference to the given []CallResource and assigns it to the Calls field.
func (o *CallResourceIndex) SetCalls(v []CallResource) {
	o.Calls = v
}

// GetEnd returns the End field value if set, zero value otherwise.
func (o *CallResourceIndex) GetEnd() int32 {
	if o == nil || IsNil(o.End) {
		var ret int32
		return ret
	}
	return *o.End
}

// GetEndOk returns a tuple with the End field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CallResourceIndex) GetEndOk() (*int32, bool) {
	if o == nil || IsNil(o.End) {
		return nil, false
	}
	return o.End, true
}

// HasEnd returns a boolean if a field has been set.
func (o *CallResourceIndex) HasEnd() bool {
	if o != nil && !IsNil(o.End) {
		return true
	}

	return false
}

// SetEnd gets a reference to the given int32 and assigns it to the End field.
func (o *CallResourceIndex) SetEnd(v int32) {
	o.End = &v
}

// GetFirstPageUri returns the FirstPageUri field value if set, zero value otherwise.
func (o *CallResourceIndex) GetFirstPageUri() string {
	if o == nil || IsNil(o.FirstPageUri) {
		var ret string
		return ret
	}
	return *o.FirstPageUri
}

// GetFirstPageUriOk returns a tuple with the FirstPageUri field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CallResourceIndex) GetFirstPageUriOk() (*string, bool) {
	if o == nil || IsNil(o.FirstPageUri) {
		return nil, false
	}
	return o.FirstPageUri, true
}

// HasFirstPageUri returns a boolean if a field has been set.
func (o *CallResourceIndex) HasFirstPageUri() bool {
	if o != nil && !IsNil(o.FirstPageUri) {
		return true
	}

	return false
}

// SetFirstPageUri gets a reference to the given string and assigns it to the FirstPageUri field.
func (o *CallResourceIndex) SetFirstPageUri(v string) {
	o.FirstPageUri = &v
}

// GetNextPageUri returns the NextPageUri field value if set, zero value otherwise.
func (o *CallResourceIndex) GetNextPageUri() string {
	if o == nil || IsNil(o.NextPageUri) {
		var ret string
		return ret
	}
	return *o.NextPageUri
}

// GetNextPageUriOk returns a tuple with the NextPageUri field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CallResourceIndex) GetNextPageUriOk() (*string, bool) {
	if o == nil || IsNil(o.NextPageUri) {
		return nil, false
	}
	return o.NextPageUri, true
}

// HasNextPageUri returns a boolean if a field has been set.
func (o *CallResourceIndex) HasNextPageUri() bool {
	if o != nil && !IsNil(o.NextPageUri) {
		return true
	}

	return false
}

// SetNextPageUri gets a reference to the given string and assigns it to the NextPageUri field.
func (o *CallResourceIndex) SetNextPageUri(v string) {
	o.NextPageUri = &v
}

// GetPage returns the Page field value if set, zero value otherwise.
func (o *CallResourceIndex) GetPage() int32 {
	if o == nil || IsNil(o.Page) {
		var ret int32
		return ret
	}
	return *o.Page
}

// GetPageOk returns a tuple with the Page field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CallResourceIndex) GetPageOk() (*int32, bool) {
	if o == nil || IsNil(o.Page) {
		return nil, false
	}
	return o.Page, true
}

// HasPage returns a boolean if a field has been set.
func (o *CallResourceIndex) HasPage() bool {
	if o != nil && !IsNil(o.Page) {
		return true
	}

	return false
}

// SetPage gets a reference to the given int32 and assigns it to the Page field.
func (o *CallResourceIndex) SetPage(v int32) {
	o.Page = &v
}

// GetPageSize returns the PageSize field value if set, zero value otherwise.
func (o *CallResourceIndex) GetPageSize() int32 {
	if o == nil || IsNil(o.PageSize) {
		var ret int32
		return ret
	}
	return *o.PageSize
}

// GetPageSizeOk returns a tuple with the PageSize field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CallResourceIndex) GetPageSizeOk() (*int32, bool) {
	if o == nil || IsNil(o.PageSize) {
		return nil, false
	}
	return o.PageSize, true
}

// HasPageSize returns a boolean if a field has been set.
func (o *CallResourceIndex) HasPageSize() bool {
	if o != nil && !IsNil(o.PageSize) {
		return true
	}

	return false
}

// SetPageSize gets a reference to the given int32 and assigns it to the PageSize field.
func (o *CallResourceIndex) SetPageSize(v int32) {
	o.PageSize = &v
}

// GetStart returns the Start field value if set, zero value otherwise.
func (o *CallResourceIndex) GetStart() int32 {
	if o == nil || IsNil(o.Start) {
		var ret int32
		return ret
	}
	return *o.Start
}

// GetStartOk returns a tuple with the Start field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CallResourceIndex) GetStartOk() (*int32, bool) {
	if o == nil || IsNil(o.Start) {
		return nil, false
	}
	return o.Start, true
}

// HasStart returns a boolean if a field has been set.
func (o *CallResourceIndex) HasStart() bool {
	if o != nil && !IsNil(o.Start) {
		return true
	}

	return false
}

// SetStart gets a reference to the given int32 and assigns it to the Start field.
func (o *CallResourceIndex) SetStart(v int32) {
	o.Start = &v
}

// GetUri returns the Uri field value if set, zero value otherwise.
func (o *CallResourceIndex) GetUri() string {
	if o == nil || IsNil(o.Uri) {
		var ret string
		return ret
	}
	return *o.Uri
}

// GetUriOk returns a tuple with the Uri field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CallResourceIndex) GetUriOk() (*string, bool) {
	if o == nil || IsNil(o.Uri) {
		return nil, false
	}
	return o.Uri, true
}

// HasUri returns a boolean if a field has been set.
func (o *CallResourceIndex) HasUri() bool {
	if o != nil && !IsNil(o.Uri) {
		return true
	}

	return false
}

// SetUri gets a reference to the given string and assigns it to the Uri field.
func (o *CallResourceIndex) SetUri(v string) {
	o.Uri = &v
}

func (o CallResourceIndex) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CallResourceIndex) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Calls) {
		toSerialize["calls"] = o.Calls
	}
	if !IsNil(o.End) {
		toSerialize["end"] = o.End
	}
	if !IsNil(o.FirstPageUri) {
		toSerialize["first_page_uri"] = o.FirstPageUri
	}
	if !IsNil(o.NextPageUri) {
		toSerialize["next_page_uri"] = o.NextPageUri
	}
	if !IsNil(o.Page) {
		toSerialize["page"] = o.Page
	}
	if !IsNil(o.PageSize) {
		toSerialize["page_size"] = o.PageSize
	}
	if !IsNil(o.Start) {
		toSerialize["start"] = o.Start
	}
	if !IsNil(o.Uri) {
		toSerialize["uri"] = o.Uri
	}
	return toSerialize, nil
}

type NullableCallResourceIndex struct {
	value *CallResourceIndex
	isSet bool
}

func (v NullableCallResourceIndex) Get() *CallResourceIndex {
	return v.value
}

func (v *NullableCallResourceIndex) Set(val *CallResourceIndex) {
	v.value = val
	v.isSet = true
}

func (v NullableCallResourceIndex) IsSet() bool {
	return v.isSet
}

func (v *NullableCallResourceIndex) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCallResourceIndex(val *CallResourceIndex) *NullableCallResourceIndex {
	return &NullableCallResourceIndex{value: val, isSet: true}
}

func (v NullableCallResourceIndex) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCallResourceIndex) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


