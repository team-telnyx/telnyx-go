...
func TestRecordingGet(t *testing.T) {
	...
	_, err := client.Recordings.Get(context.TODO(), "recording_id")
	if err != nil {
...
func TestRecordingListWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
...
func TestRecordingDelete(t *testing.T) {
	...
	_, err := client.Recordings.Delete(context.TODO(), "recording_id")
	if err != nil {
		var apierr *telnyx.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
