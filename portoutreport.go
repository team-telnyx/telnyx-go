...
func (r *PortoutReportService) List(ctx context.Context, query PortoutReportListParams, opts ...option.RequestOption) (res *PortoutReportListResponse, err error) {
	opts = append(r.Options[:], opts...)
	path := "portouts/reports"
...
type PortoutReportListResponse struct {
	Data []PortoutReport `json:"data"`
...
// Returns the unmodified JSON received from the API
func (r PortoutReportListResponse) RawJSON() string { return r.JSON.raw }
func (r *PortoutReportListResponse) UnmarshalJSON(data []byte) error {
	return apijson.UnmarshalRoot(data, r)
}
...
type PortoutReportListParams struct {
	...
	// page[size]
	Page PortoutReportListParamsPage `query:"page,omitzero" json:"-"`
	paramObj
}
...
func init() {
	apijson.RegisterFieldValidator[PortoutReportListParamsFilter](
...
// Consolidated page parameter (deepObject style). Originally: page[number],
// page[size]
type PortoutReportListParamsPage struct {
...
// URLQuery serializes [PortoutReportListParamsPage]'s query parameters as
// `url.Values`.
func (r PortoutReportListParamsPage) URLQuery() (v url.Values, err error) {
	return apiquery.MarshalWithSettings(r, apiquery.QuerySettings{
		ArrayFormat:  apiquery.ArrayQueryFormatComma,
		NestedFormat: apiquery.NestedQueryFormatBrackets,
	})
