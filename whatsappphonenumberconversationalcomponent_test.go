...
func TestWhatsappPhoneNumberConversationalComponentList(t *testing.T) {
	...
	_, err := client.Whatsapp.PhoneNumbers.ConversationalComponents.List(context.TODO(), "phone_number")
	if err != nil {
...
func TestWhatsappPhoneNumberConversationalComponentPatchAllWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
